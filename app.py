...
#function to retirece a post from the database
...
def index():
    ...
    #execute a query to read all posts from the pists table in the database
    posts = conn.execute('SELECT * FROM posts').fetchall()

    #close the connection
    conn.close()
    #send the posts to the index.html to be dispalyed
...
def create():
    ...
    if request.method == 'POST':
        ...
        #display an error if title or conntent is not submitted
...
#create a route to edit a post. Load the page with get or post method
...
def edit(id):
    ...
    #determine if the page was requested with the GET or POST
    ...
    if request.method == 'POST':
        ...
        #if mot title or content, flash the error message
        if not title:
            flash('Title is required')
        elif not content:
            flash('Content is required')
